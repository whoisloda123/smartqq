...
import com.scienjus.smartqq.utils.SslUtils;
...
public class SmartQQClient implements Closeable {
    ...
    private void login() {
        ...
        log.info("登录成功！！，{}，欢迎！", userInfo.getNick());
    }
    ...
    private void getQRCode() {
        ...
        try {
            ...
            log.error("[登录]二维码保存异常", e);
            throw new IllegalStateException("二维码保存异常");
        }
        ...
        log.info("[登录]二维码已保存成功，保存在文件中:{}，请打开手机QQ并扫描二维码", filePath);
    }
            ...
            if (result.contains("成功")) {
                for (String content : result.split("','")) {
                    if (content.startsWith("http")) {
                        log.info("[登录]正在登录，请稍后...");

                        return content;
                    }
                }
            } else if (result.contains("已失效")) {
                log.info("[登录]二维码已失效，尝试重新获取二维码...");
                getQRCode();
            }
        }
    ...
    private void getVfwebqq() {
        ...
        log.info("[登录]获取vfwebqq成功, ptwebqq:{}，vfwebqq：{}", ptwebqq, vfwebqq);
    }
    ...
    private void getUinAndPsessionid() {
        ...
        log.info("获取uin和psessionid成功，uin：{}，psessionid：{}", uin, psessionid);
    }
    ...
    private static void checkSendMsgResult(Response<String> response) {
        if (response.getStatusCode() != 200) {
            log.error("发送失败，Http返回码:{}", response.getStatusCode());
        }
        ...
        if (errCode != null && errCode == 0) {
            ...
            log.error("发送失败，Api返回码:{}", json.getInteger("retcode"));
        }
    }
